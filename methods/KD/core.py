...
def build_loaders(args: argparse.Namespace, device: torch.device) -> tuple[Any, Any]:
    if args.dataset != "cifar100":
        raise NotImplementedError(
            "V2 currently exposes the verified paired-resolution pipeline for "
            "CIFAR-100 only. Flowers-102 and Chaoyang are added after this stage."
        )
...
def student_view_to_teacher_view(images: torch.Tensor) -> torch.Tensor:
    ...
    student_mean = images.new_tensor(CIFAR100_MEAN).view(1, 3, 1, 1)
    student_std = images.new_tensor(CIFAR100_STD).view(1, 3, 1, 1)
...
def train_one_epoch(
    ...
    for images, targets in loader:
        ...
        with torch.no_grad(), autocast_context(amp_enabled):
            teacher_logits = teacher(student_view_to_teacher_view(images))
