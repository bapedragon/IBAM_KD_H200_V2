...
class ContrastiveSampleDataset(Dataset[Any]):
    ...
    def __init__(
        ...
        *,
        ...
        # Tiny smoke subsets need not contain every dataset class. Every sample
        # still has at least itself as a positive, and absent classes can never
        # be selected as the current target. Full CIFAR-100 timing/training
...
def forward_teacher_with_rep(
    ...
    representation = torch.flatten(teacher.head.avg_pool(feature), 1)
    logits = teacher.head.fc(representation)
...
def main() -> None:
    ...
    with torch.no_grad():
        student_probe = torch.zeros(
            2, 3, args.image_size, args.image_size, device=device
        )
        teacher_probe = student_view_to_teacher_view(student_probe, args.dataset)
        teacher_rep, _ = forward_teacher_with_rep(teacher, teacher_probe)
        student_rep, _ = forward_student_with_rep(student, student_probe)
        ...
        for images, targets, indexes, sampled_indexes in train_loader:
            ...
            with torch.no_grad(), autocast_context(amp_enabled):
                teacher_representation, _ = forward_teacher_with_rep(
                    teacher, student_view_to_teacher_view(images, args.dataset)
                )
